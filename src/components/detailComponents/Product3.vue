<template>
  <div class="sales-board">
    <div class="sales-board-intro">
      <h3>小米空气净化套装</h3>
      <p>
        此款小米净化器套装包含小米空气净化器一个，一只空气净化器滤芯替换装。新研发出的空气增压系统，
        10分钟让房间空气清新。 高效净化滤芯360°进风，经过层层净化，不放过任何有害物质
      </p>
    </div>
    <div class="sales-board-form">
      ...
      <div class="sales-board-info">
        ...
        <div class="right">
          <checkboxChoose
            :selections="placesTypes"
            @selectChange="onParamChange('proPlaces', $event)"
          ></checkboxChoose>
        </div>
      </div>
      <div class="sales-board-info">
    ...
    <div class="sales-board-description">
      <h3>产品说明</h3>
      <p>
        空气增压系统改善室内空气质量，加速空间空气流通；清洁滤芯可吸附净化环境中PM2.5、甲醛、烟尘、毛发等污染物；
        远程APP遥控开启，环境空气不良自动激活，WIFI智能控制，操作便捷。
      </p>
      <p>
        全新的核心算法，3键操作简单明了，手握防滑设计，不易脱落方便使用。实现甲醛和PM2.5同检先进技术，
        数据精度高性能稳响应速度快特点，有效降低了测试仪的成本。
      </p>
      <p>
        拥有高雅颜值的家用空气净化器，为你开启洁净的呼吸生活。触屏的操控方式，空气质量指数一目了然。
        它采用全新3D循环净化，四重过滤无残留，更有高效的负离子发生器，让你享受森林般的呼吸。
      </p>
    </div>
      ...
      <div slot="content-info">
        <table class="payDialog-table">
          ...
          <tbody>
            <td>{{proNumber}}</td>
            <td>
              <span v-for="(item, index) in proPlaces" v-if="!item.name">{{item.name}}</span>
            </td>
            <td>一年</td>
...
import CheckboxChoose from "../../components/detailSelectionComponents/checkboxChoose";
